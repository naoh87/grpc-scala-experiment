...
class Network extends Actor {
  ...
  val actives = mutable.HashMap.empty[String, ActorRef]
  ...
  override def receive: Receive = {
    ...
    case msg: pn.ServerLeave =>
      msg.server
        ...
        .foreach(_ ! msg)
    case msg@Open(name) =>
      find(name) forward msg
  }

  def find(name: String) = actives.getOrElseUpdate(name, create(name))

  def create(name: String) = context.actorOf(ServerLocationActor.props(name), name)
}
